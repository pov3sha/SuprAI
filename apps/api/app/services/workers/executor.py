...
def execute_worker_task(task_id: str) -> dict:
    ...
    try:
        ...
        # 2. Get Ollama AI Provider for Worker Role
        ...
        # 3. Gather Real Document Context
        ...
        # 4. REAL OLLAMA INTER-AGENT COMMUNICATION (NO HARDCODING)
        # Step A: Worker asks a real question generated by Ollama
        question_prompt = (
            f"You are the {agent_name}. Your objective is: {task.objective}\n"
            f"Document snippet: {doc_context[:500] if doc_context else 'No document.'}\n"
            "Ask a specific 1-sentence question to the Manager requesting clarification on how to proceed."
        )
        try:
            q_res = ai_provider.generate(prompt=question_prompt, temperature=0.3)
            question_text = q_res.content.strip() if q_res and q_res.content else f"Manager, how should I prioritize the analysis for {task.objective[:40]}?"
        except Exception:
            question_text = f"Manager, how should I prioritize the analysis for {task.objective[:40]}?"
        ...
        # Step B: Manager AI answers with a real Ollama response
        clarification_prompt = (
            f"You are the Lead Manager AI.\n"
            f"Your worker '{agent_name}' asked: \"{question_text}\"\n"
            f"Objective: {task.objective}\n"
            "Provide a direct, authoritative 1-sentence strategic direction response."
        )
        try:
            c_res = manager_provider.generate(prompt=clarification_prompt, temperature=0.3)
            clarification_text = c_res.content.strip() if c_res and c_res.content else "Prioritize high-impact findings and risk mitigations."
        except Exception:
            clarification_text = "Prioritize high-impact findings and structural mitigations."
        ...
        event_publisher.publish_event(
            ...
            payload={
                "task_id": task.id,
                "agent_name": agent_name,
                "message": f"Acknowledged Manager guidance: \"{clarification_text[:80]}\". Continuing execution.",
                "timestamp": time.time()
            }
        )

        # 5. REAL WORKER EXECUTION VIA OLLAMA
        prompt = (
            f"WORKER ROLE: {agent_name}\n"
            f"OBJECTIVE: {task.objective}\n"
            f"MANAGER GUIDANCE: {clarification_text}\n"
            f"DOCUMENT CONTEXT:\n{doc_context[:1500] if doc_context else 'No document attached.'}\n\n"
            "Provide a concise, analytical 2-3 sentence findings summary based on the document."
        )
        ...
        try:
            ...
            worker_summary = response.content.strip() if response and response.content else f"Completed worker analysis for {task.objective}"
            ...
            logger.warning(f"Worker generation fallback for {agent_name}: {e}")
            worker_summary = f"Analysis completed for {task.objective} under Manager guidance."
